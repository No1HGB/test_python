...
def preprocessing_data(df: pd.DataFrame):
    ...
    df["rank01"] = df.groupby(by="date")["fcf_rank"].rank(
        ascending=True, method="first"
    )
    # FCF(QSUM) 의 절대금액이 큰 회사를 고름 (rank01 에서 FCF(QSUM) 이 작은 회사도 뽑힐 수 있으므로
    df["rank02"] = df.groupby(by="date")["FCF(QSUM)"].rank(
        ascending=False, method="first"
    )
    # 최종적으로 회사의 시총이 크면서 FCF(QSUM) 도 큰 회사를 뽑음
    df["total_rankvalue"] = df["rank01"] + df["rank02"]
